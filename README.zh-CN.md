...
- 管理后台：密钥管理、节点改名/归属地修改、管理员用户名修改、生成客户端一键安装命令
...
|---|---|---|
| `PORT` | `8080` | 监听端口 |
| `PROBE_ADMIN_USER` | `admin` | 初始管理员用户名（后台保存的用户名优先于此值） |
...
| `GET/POST /api/admin/settings` | 会话 | 账号设置（修改管理员用户名） |
...
测试为纯标准库实现，共 16 个端到端用例（登录、限流、密钥、上报、节点管理、吊销、登出、静态白名单、XFF 解析、封禁/解封、账号设置）。CI（GitHub Actions）执行 `py_compile`、Python 3.10/3.12 测试和 ShellCheck，工作流文件见 `.github/workflows/ci.yml`。
