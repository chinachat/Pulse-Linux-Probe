...
| `GET/POST /api/admin/nodes`、`DELETE /api/admin/nodes/{id}` | 会话 | 节点管理 |
...
测试为纯标准库实现，共 14 个端到端用例（登录、限流、密钥、上报、节点管理、吊销、登出、静态白名单、XFF 解析）。CI（GitHub Actions）执行 `py_compile`、Python 3.10/3.12 测试和 ShellCheck，工作流文件见 `.github/workflows/ci.yml`。
